...
create an event object. All event objects will be handed off to a 'Hub' which
handles the rest.

The hub has 2 jobs, synchrnization, and event hooks. The hub manages state, and
directs events to where they need to go. In this system the hub is the only
component that acts as a singleton. The hub should be as minimal as possible to
avoid a monolithic singleton.

There should be no restrictions against creating multiple instances of the hub
object. There should also be a way to get a canonical hub to which events
should be sent by default. In other words this is where a singleton or similar
